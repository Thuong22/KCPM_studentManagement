       ...
       (
        ...
        private void btnLogin_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                DataProvider.account = new Account();
                bool flag = DataProvider.account.GetMyAccount("SELECT * FROM `account` WHERE `Username`= '" + user + "' AND `Password`='" + pass + "'");
                if (flag)
                {
                    if (DataProvider.account.type == 1)
                    {
                        Thread thread = new Thread(new ThreadStart(ShowAdminForm));
                        thread.Start();
                    }
                    else if (DataProvider.account.type == 2)
                    {
