...
namespace Transparent_Form
{
    public partial class PrintStudentForm : Form
    {
        ...
        private void PrintStudent_Load(object sender, EventArgs e)
        {
            ...
            LoadStudentList("SELECT AccId, AccFirstName, AccLastName, Birthdate, Gender, Phone, Address, Photo FROM `account` WHERE Type = 2");
        }
        ...
        private void cbbGender_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (cbbGender.Text == "All")
                query = "SSELECT AccId, AccFirstName, AccLastName, Birthdate, Gender, Phone, Address, Photo FROM `account` WHERE Type = 2";
            else
                query = "SELECT AccId, AccFirstName, AccLastName, Birthdate, Gender, Phone, Address, Photo FROM `account` WHERE Type = 2 AND `Gender`='" + cbbGender.Text + "'";
